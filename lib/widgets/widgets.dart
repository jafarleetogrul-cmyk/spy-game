...
class GradientButton extends StatelessWidget {
  ...
  const GradientButton({
    ...
    this.gradient = AppTheme.primaryGradient, this.width, this.icon,
  });
...
class CountdownTimerWidget extends StatelessWidget {
  ...
  const CountdownTimerWidget({
    super.key, required this.seconds, this.isWarning = false, this.expired = false,
  });
...
class PlayerTile extends StatelessWidget {
  ...
  static const _avatarColors = [
    Color(0xFF6C63FF), Color(0xFFFF6584), Color(0xFF43E97B), Color(0xFFFFD166),
    Color(0xFF4CC9F0), Color(0xFFF77F00), Color(0xFF7B2D8B), Color(0xFF06D6A0),
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        CircleAvatar(
          radius: 20, backgroundColor: avatarColor,
...
class ScoreRow extends StatelessWidget {
  ...
  const ScoreRow({super.key, required this.name, required this.spyWins, required this.civilWins, required this.totalPoints, required this.rank});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: rank == 1 ? AppTheme.primary.withOpacity(0.2) : AppTheme.card,
        borderRadius: BorderRadius.circular(14),
        border: Border.all(color: rank == 1 ? AppTheme.primary.withOpacity(0.4) : Colors.white.withOpacity(0.06)),
      ),
      child: Row(children: [
        Text(rank <= 3 ? medals[rank - 1] : '$rank.', style: const TextStyle(fontSize: 20)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: const TextStyle(color: AppTheme.textMain, fontWeight: FontWeight.w600, fontSize: 15)),
          const SizedBox(height: 2),
          Text('🕵️ $spyWins  👥 $civilWins', style: const TextStyle(color: AppTheme.textSub, fontSize: 12)),
        ])),
        Text('$totalPoints xal', style: const TextStyle(color: AppTheme.accent, fontWeight: FontWeight.bold, fontSize: 16)),
      ]),
    ).animate().fadeIn(duration: 400.ms, delay: (rank * 80).ms).slideY(begin: 0.1);
  }
}
