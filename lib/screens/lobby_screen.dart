...
class _LobbyScreenState extends State<LobbyScreen> {
  ...
  Future<void> _startGame() async {
    ...
    try {
      await gp.startGame();
      if (mounted) { gp.stopPolling(); await gp.fetchMyCard(); if (mounted) Navigator.pushReplacementNamed(context, '/role_card'); }
    } catch (_) {}
  }
      ...
      body: Container(
        ...
        child: SafeArea(child: LoadingOverlay(
          ...
          child: CustomScrollView(slivers: [
            SliverAppBar(backgroundColor: Colors.transparent, automaticallyImplyLeading: false,
              title: Text('Lobby – ${gp.session?.roomCode ?? ''}', style: const TextStyle(letterSpacing: 2)),
              actions: [IconButton(icon: const Icon(Icons.exit_to_app, color: Colors.white70),
              ...
              sliver: SliverList(delegate: SliverChildListDelegate([
                ...
                GlassCard(
                  ...
                  child: Column(children: [
                    Row(children: [
                      const Icon(Icons.qr_code_2, color: AppTheme.primary, size: 22), const SizedBox(width: 8),
                      const Expanded(child: Text('QR Kodu', style: TextStyle(color: AppTheme.textMain, fontWeight: FontWeight.w600, fontSize: 16))),
                      GestureDetector(
                        onTap: () { Clipboard.setData(ClipboardData(text: gp.session?.roomCode ?? ''));
                        ...
                        child: Container(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            Text(gp.session?.roomCode ?? '', style: const TextStyle(color: AppTheme.primary, fontWeight: FontWeight.w800, fontSize: 14, letterSpacing: 2)),
                ...
                if (gp.isHost)
                  Column(children: [
                    if (players.length < 3) Padding(padding: const EdgeInsets.only(bottom: 12),
                    ...
                    GradientButton(label: L.t('start_game'), icon: Icons.play_arrow_rounded,
                      onTap: players.length >= 3 ? _startGame : null),
